...
// MARK: - 文件输出目标
public final class SoloFileDestination {
    /// 目标唯一标识符
    ...
    public init?(
        ...
        minimumLevel: SoloLogLevel = .debug,
        maxFileSize: Int = 0,
        ...
        do {
            ...
            os_log(.error, "SoloFileDestination: 创建目录失败 %{public}@: %{public}@", dir.path, error.localizedDescription)
            return nil
        }
        ...
        guard let handle = FileHandle(forWritingAtPath: filePath) else {
            os_log(.error, "SoloFileDestination: 无法打开文件 %{public}@", filePath)
            return nil
        }
...
// MARK: - SoloLogDestination
extension SoloFileDestination: SoloLogDestination {
    public func log(context: SoloLogContext) {
...
// MARK: - 轮转
private extension SoloFileDestination {
