...
open class DySheetView: UIView {
    ...
    /// 遮罩层
    private lazy var shadeView = DyView.view()

    /// 内容容器（从底部弹出的部分）
    public lazy var contentView = DyView.view()
...
@objc extension DySheetView {
    ...
    open func show(in container: UIView? = nil) {
        ...
        self.contentView
            .dy
            .left((container.dy_width - self.contentView.dy_width) / 2)
            .top(self.dy_height - self.contentView.dy_height)

        // 设置初始化状态
        self.shadeView.alpha = 0.01
        self.contentView.alpha = 0.01
        self.contentView.transform = CGAffineTransform(translationX: 0, y: self.contentView.dy_height)
