...
open class SoloNavigationController: UINavigationController {
...
// MARK: - SoloSetupable
@objc extension SoloNavigationController: SoloSetupable {
    /// 控制器初始化样式
    open func setupUI() {
        self
            .solo
...
// MARK: - 控制器设置
@objc extension SoloNavigationController {
    // MARK: - 屏幕旋转
...
@objc extension SoloNavigationController: UINavigationControllerDelegate {
    /// 栈顶控制器即将显示
...
@objc extension SoloNavigationController: UIGestureRecognizerDelegate {
    /// 是否可以侧滑返回
    open func gestureRecognizerShouldBegin(_ gestureRecognizer: UIGestureRecognizer) -> Bool {
        ...
        // 如果是SoloViewController类或子类
        if let lastChildren = children.last as? SoloViewController {
