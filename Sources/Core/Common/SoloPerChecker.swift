...
// MARK: - 权限请求结果
public enum SoloPerReqResult {
    /// 用户已授权
    case authorized
    /// 用户或系统拒绝了权限请求
    case denied(reason: SoloDenialReason)
}

// MARK: - 权限拒绝原因
public enum SoloDenialReason {
    /// 用户明确拒绝
    case userDenied
    /// 系统策略限制(如家长控制、设备管理)
...
public enum SoloPerReqType {
    ...
    case adTracking
    /// 位置
    case location(LocationSoloPerReqType)
    /// 通知
    case notification(options: UNAuthorizationOptions)

    /// 位置权限请求类型
    public enum LocationSoloPerReqType {
        /// 使用时
        case whenInUse
        /// 一直
        case always
...
// MARK: - 权限管理器
public final class SoloPerChecker: NSObject {
    ...
    private var latestLocationCallback: SoloAction1<SoloPerReqResult>?
    private let locationCallbackLock = NSLock()

    public static let shared = SoloPerChecker()
...
public extension SoloPerChecker {
    /// 查询指定权限的当前授权状态。
    /// - Parameters:
    ///   - type: 权限类型。
    ///   - completion: 可选回调。
    ...
    @discardableResult
    func checkStatus(for type: SoloPerReqType, completion: SoloAction1<SoloPerStatus>? = nil) -> SoloPerStatus {
        switch type {
        case .photoLibrary:
    ...
    func request(_ type: SoloPerReqType, completion: @escaping SoloAction1<SoloPerReqResult>) {
        switch type {
...
// MARK: - 通知权限
private extension SoloPerChecker {
    /// 异步获取通知设置状态。
    ...
    ///   任一未开启即视为 `.denied`。
    func checkNotificationSettings(for options: UNAuthorizationOptions, completion: @escaping SoloAction1<SoloPerStatus>) {
        UNUserNotificationCenter.current().getNotificationSettings { settings in
            let status: SoloPerStatus
    ...
    func requestNotification(options: UNAuthorizationOptions, completion: @escaping SoloAction1<SoloPerReqResult>) {
        UNUserNotificationCenter.current().requestAuthorization(options: options) { granted, error in
            let result: SoloPerReqResult = error.map { .denied(reason: .error($0)) }
...
private extension SoloPerChecker {
    ...
    func checkPhotoLibrary() -> SoloPerStatus {
    ...
    func requestPhotoLibrary(completion: @escaping SoloAction1<SoloPerReqResult>) {
        let handler: SoloAction1<PHAuthorizationStatus> = { status in
            let result: SoloPerReqResult = {
                switch status {
                case .authorized, .limited: return .authorized
...
private extension SoloPerChecker {
    /// 查询相机授权状态。
    func checkCamera() -> SoloPerStatus {
    ...
    func requestCamera(completion: @escaping SoloAction1<SoloPerReqResult>) {
        AVCaptureDevice.requestAccess(for: .video) { granted in
            let result: SoloPerReqResult = granted ? .authorized : .denied(reason: .userDenied)
            DispatchQueue.main.async { completion(result) }
        }
    }
}
...
private extension SoloPerChecker {
    /// 查询麦克风授权状态。
    func checkMicrophone() -> SoloPerStatus {
    ...
    func requestMicrophone(completion: @escaping SoloAction1<SoloPerReqResult>) {
...
private extension SoloPerChecker {
    ...
    func checkContacts() -> SoloPerStatus {
    ...
    /// 请求通讯录权限。
    func requestContacts(completion: @escaping SoloAction1<SoloPerReqResult>) {
...
private extension SoloPerChecker {
    ...
    func checkAdTracking() -> SoloPerStatus {
        if #available(iOS 14, *) {
    ...
    func requestAdTracking(completion: @escaping SoloAction1<SoloPerReqResult>) {
        if #available(iOS 14, *) {
            ATTrackingManager.requestTrackingAuthorization { status in
                let result: SoloPerReqResult = (status == .authorized)
...
private extension SoloPerChecker {
    /// 查询定位授权状态。
    func checkLocation() -> SoloPerStatus {
    ...
    func requestLocation(type: SoloPerReqType.LocationSoloPerReqType, completion: @escaping SoloAction1<SoloPerReqResult>) {
    ...
    /// 将系统定位授权状态映射为 `SoloPerReqResult`。
    /// - Note: `.restricted`(如家长控制)映射为 `systemRestricted`,与用户主动拒绝区分开。
    func locationResult(for status: CLAuthorizationStatus) -> SoloPerReqResult {
        switch status {
        case .authorizedAlways, .authorizedWhenInUse: return .authorized
