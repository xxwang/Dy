...
public final class SoloQueue: @unchecked Sendable {
    private var onceTokens: Set<String> = []
    private let onceLock = NSLock()
    /// 复用的私有串行队列，避免 `executeSerially` 每次调用都新建队列
    private let serialTaskQueue = DispatchQueue(label: "com.solo.serial-task-queue")

    public static let shared = SoloQueue()
    private init() {}
}

// MARK: - 异步执行
public extension SoloQueue {
    /// 在主线程异步执行指定任务
    ///
    ...
    /// - Example:
    ///   ```swift
    ///   SoloQueue.shared.asyncOnMain {
    ///       self.statusLabel.text = "加载完成"
    ///   }
    ///   ```
    func asyncOnMain(_ task: @escaping SoloAction) {
    ...
    /// - Example:
    ///   ```swift
    ///   SoloQueue.shared.asyncOnGlobal(qos: .userInitiated) {
    ...
    func asyncOnGlobal(
        ...
        execute task: @escaping SoloAction
    ) {
...
public extension SoloQueue {
    ...
    /// - Example:
    ///   ```swift
    ///   let tasks: [SoloAction] = [
    ///       { print("步骤1：初始化") },
    ///       { Thread.sleep(forTimeInterval: 0.2); print("步骤2：处理数据") }
    ///   ]
    ///   SoloQueue.shared.executeSerially(tasks) {
    ///       print("所有串行任务完成，刷新界面")
    ///   }
    ///   ```
    func executeSerially(
        _ tasks: [SoloAction],
        then completion: @escaping SoloAction
    ) {
    ...
    ///   ]
    ///   SoloQueue.shared.executeConcurrently(tasks) {
    ///       self.reloadData()
    ///   }
    ///   ```
...
public extension SoloQueue {
    ...
    ///   timer = SoloQueue.shared.timer(every: 1.0) { _ in
    ///       print("每秒触发一次")
    ///   }
    ///   // 停止定时器：timer?.cancel()
    ///   ```
    ...
    func timer(
        ...
        handler: @escaping SoloAction1<DispatchSourceTimer>
    ) -> DispatchSourceTimer {
    ...
    /// - Example:
    ///   ```swift
    ///   SoloQueue.shared.countdownTimer(every: 1.0, times: 3) { _, remaining in
    ...
    func countdownTimer(
        every interval: TimeInterval,
        times count: Int,
        handler: @escaping SoloAction2<DispatchSourceTimer, Int>
    ) -> DispatchSourceTimer? {
...
public extension SoloQueue {
    ...
    /// - Example:
    ///   ```swift
    ///   SoloQueue.shared.delayed(2.0, on: .main) {
    ///       self.showToast("操作成功")
    ///   }
    ///   ```
    func delayed(
    ...
    /// - Example:
    ///   ```swift
    ///   let item = SoloQueue.shared.backgroundDelayed(3.0) {
    ///       print("后台任务执行中...")
    ///   } then: {
    ///       print("回到主线程更新 UI")
    ///   }
    ...
    func backgroundDelayed(
        _ delay: TimeInterval,
        execute work: @escaping SoloAction,
        then completion: SoloAction? = nil
    ) -> DispatchWorkItem {
...
public extension SoloQueue {
    ...
    /// - Example:
    ///   ```swift
    ///   SoloQueue.shared.executeOnce(token: "app.setup.analytics") {
    ///       Analytics.shared.setup()
    ///   }
    ///   ```
    func executeOnce(token: String, execute work: SoloAction) {
