...
public extension UIView {
    ...
    var leadingConstraint: NSLayoutConstraint? {
        constraint(for: .leading)
    }

    /// 获取当前视图的第一个 trailing 约束
    var trailingConstraint: NSLayoutConstraint? {
        constraint(for: .trailing)
    }
    ...
    var centerXConstraint: NSLayoutConstraint? {
        constraint(for: .centerX)
    }

    /// 获取当前视图的第一个 centerY 约束
    var centerYConstraint: NSLayoutConstraint? {
        constraint(for: .centerY)
    }
    ...
    private func constraint(for attribute: NSLayoutConstraint.Attribute) -> NSLayoutConstraint? {
        ...
        if let constraint = constraints.first(where: { constraint in
            isConstraint(constraint, affecting: self, with: attribute)
        }) {
            return constraint
        }
...
public extension UIView {
    ...
    /// - Example:
    ///   ```swift
    ///   subview.fillSuperview(insets: .init(top: 10, left: 10, bottom: 10, right: 10))
    ///   ```
    @discardableResult
    func fillSuperview(
    ...
    /// - Example:
    ///   ```swift
    ///   view.centerInSuperview(offset: CGPoint(x: 0, y: 10))
    ///   ```
    @discardableResult
