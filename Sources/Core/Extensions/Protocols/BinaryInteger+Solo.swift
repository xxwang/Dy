...
extension Int: SoloExtension {}

// MARK: - 类型转换
public extension SoloWrapper where Base: BinaryInteger {
    /// 转换为 `Bool`
    func toBool() -> Bool {
        base > 0
    }
...
public extension SoloWrapper where Base: BinaryInteger {
    ...
    /// - Example:
    ///   ```swift
    ///     let radians = 180.solo.toRadians() // ≈ π
    ///     ```
    ...
    /// - Example:
    ///   ```swift
    ///     let degrees = Int.pi.solo.toDegrees() // ≈ 180.0
    ///     ```
...
public extension SoloWrapper where Base: BinaryInteger {
    ...
    /// - Example:
    ///   ```swift
    ///     let size = (1_572_864 as Int).solo.storageUnit() // "1.50 MB"
    ///     ```
    ...
    /// - Example:
    ///   ```swift
    ///     let roman = 1987.solo.toRomanNumeral() // "MCMLXXXVII"
    ///     ```
...
public extension SoloWrapper where Base: BinaryInteger {
    ...
    /// - Example:
    ///   ```swift
    ///     let date = 1_609_459_200.solo.date() // 2021-01-01 UTC
    ///     let dateMs = 1_609_459_200_000.solo.date(isUnix: false) // 同上
    ///     ```
    ...
    /// - Example:
    ///   ```swift
    ///     let full = 3661.solo.durationString() // "01:01:01"
    ///     let minSec = 3661.solo.durationString(component: .minute) // "61:01"
    ///     ```
...
public extension SoloWrapper where Base: BinaryInteger {
    ...
    /// - Example:
    ///   ```swift
    ///     let range = 10.solo.range(from: 5) // 5..<10
    ///     ```
    ...
    /// - Example:
    ///   ```swift
    ///     let range = 5.solo.range(to: 10) // 5..<10
    ///     ```
