...
public extension DyWrapper where Base: RangeExpression, Base.Bound == Int {
    /// 将区间转换为包含所有整数的数组
    ...
    ///   ```swift
    ///   let range = 1...5
    ///   print(range.dy.collect) // [1, 2, 3, 4, 5]
    ///   ```
    var collect: [Int] {
    ...
    ///   ```swift
    ///   let range = 1...5
    ///   print(range.dy.containsAll([2, 3])) // true
    ///   print(range.dy.containsAll([2, 6])) // false
    ///   ```
