...
extension Float: SoloExtension {}
extension Double: SoloExtension {}
extension CGFloat: SoloExtension {}

// MARK: - 类型转换
public extension SoloWrapper where Base: BinaryFloatingPoint {
    /// 将当前浮点数值转换为布尔值
    func toBool() -> Bool {
        base > 0
    }
...
public extension SoloWrapper where Base: BinaryFloatingPoint {
    ...
    ///     print(value.solo.truncate(places: 2)) // 5.67
    ///
    ///     let negative: Double = -5.6789
    ///     print(negative.solo.truncate(places: 2)) // -5.67
    ///     ```
    ...
    ///   ```swift
    ///     let value: Double = 5.6789
    ///     print(value.solo.round(places: 2)) // 5.68
    ///     ```
    ...
    ///     print(value.solo.rounded(places: 2, rule: .down)) // 5.67
    ///     ```
