...
public extension UIButton {
    ...
    /// 按钮的常用状态
    var solo_allStates: [UIControl.State] {
...
extension UIButton {
    ...
    override open func point(inside point: CGPoint, with event: UIEvent?) -> Bool {
        let expandedRect = self.solo_expandedRect()
        // 如果没有扩展范围,则使用原始范围
    ...
    func solo_expandedRect() -> CGRect {
        if let expandSize: CGFloat = self.solo_GetAO(forKey: &SoloKeys.expandSizeKey) {
            return CGRect(
    ...
    /// - Returns: `Self`
    @discardableResult
    func solo_expandClickArea(_ size: CGFloat = 10) -> Self {
        self.solo_SetAO(size, forKey: &UIButton.SoloKeys.expandSizeKey)
        return self
    }
}
...
public extension UIButton {
    ...
    /// - Returns: 标题的`size`
    func solo_viewSize(maxWidth: CGFloat? = nil) -> CGSize {
        let maxWidth = maxWidth ?? SoloScreen.screenWidth
        return if let currentAttributedTitle = self.currentAttributedTitle {
            currentAttributedTitle.solo_viewSize(maxWidth: maxWidth)
        } else {
            self.titleLabel?.solo_viewSize(maxWidth: maxWidth) ?? .zero
        }
    }
}
