...
public extension UIColor {
    ...
    convenience init(hex: String, alpha: CGFloat = 1.0) {
        ...
        self.init(
            ...
            green: g / 255.0,
            blue: b / 255.0,
            alpha: alpha.solo_clamped(to: 0 ... 1)
        )
    }
    ...
    convenience init?(argbHex: String) {
        ...
        self.init(red: r, green: g, blue: b, alpha: a.solo_clamped(to: 0 ... 1))
    }
    ...
    convenience init(r: CGFloat, g: CGFloat, b: CGFloat, alpha: CGFloat = 1.0) {
        self.init(
            red: (r.solo_clamped(to: 0 ... 255)) / 255.0,
            green: (g.solo_clamped(to: 0 ... 255)) / 255.0,
            blue: (b.solo_clamped(to: 0 ... 255)) / 255.0,
            alpha: alpha.solo_clamped(to: 0 ... 1)
        )
    }
...
public extension UIColor {
    /// 生成一个随机颜色
    static var solo_random: UIColor {
...
public extension UIColor {
    ...
    func solo_alpha(_ alpha: CGFloat) -> UIColor {
...
public extension UIColor {
    ...
    var solo_rgbaComponents: (red: CGFloat, green: CGFloat, blue: CGFloat, alpha: CGFloat) {
    ...
    var solo_rgbComponents: (red: CGFloat, green: CGFloat, blue: CGFloat) {
        let (r, g, b, _) = self.solo_rgbaComponents
        return (r, g, b)
    }

    /// 安全获取 `RGB` 分量(0 ～ 255 整数)
    /// - Returns: `(red, green, blue)` 元组(Int,范围 0–255),若无法转换则返回 `nil`
    var solo_rgbIntComponents: (red: Int, green: Int, blue: Int)? {
        let rgba = self.solo_rgbaComponents
        return (
    ...
    var solo_rgbFloatComponents: (red: CGFloat, green: CGFloat, blue: CGFloat) {
        let rgba = self.solo_rgbaComponents
...
public extension UIColor {
    ...
    var solo_hsbaComponents: (hue: CGFloat, saturation: CGFloat, brightness: CGFloat, alpha: CGFloat) {
...
public extension UIColor {
    /// 获取红色分量(0.0 ～ 1.0)
    var solo_redComponent: CGFloat? {
        return self.solo_rgbaComponents.red
    }

    /// 获取绿色分量(0.0 ～ 1.0)
    var solo_greenComponent: CGFloat? {
        return self.solo_rgbaComponents.green
    }

    /// 获取蓝色分量(0.0 ～ 1.0)
    var solo_blueComponent: CGFloat? {
        return self.solo_rgbaComponents.blue
    }

    /// 获取透明度分量(0.0 ～ 1.0)
    var solo_alphaComponent: CGFloat? {
        return self.solo_rgbaComponents.alpha
    }

    /// 获取色相分量(0.0 ～ 1.0)
    var solo_hueComponent: CGFloat? {
        return self.solo_hsbaComponents.hue
    }

    /// 获取饱和度分量(0.0 ～ 1.0)
    var solo_saturationComponent: CGFloat? {
        return self.solo_hsbaComponents.saturation
    }

    /// 获取亮度分量(0.0 ～ 1.0)
    var solo_brightnessComponent: CGFloat? {
        return self.solo_hsbaComponents.brightness
    }
}
...
public extension UIColor {
    ...
    ///   - alpha: 透明度
    /// - Returns: `UIColor`
    static func solo_color(from hex: String, alpha: CGFloat = 1.0) -> UIColor {
        return UIColor(hex: hex, alpha: alpha)
    }

    /// 转换为`CIColor`
    func solo_cIColor() -> CoreImage.CIColor {
    ...
    /// - Parameter size: 图片尺寸
    /// - Returns: `UIImage?`
    func solo_uIImage(ofSize size: CGSize) -> UIImage? {
    ...
    func solo_hexString(prefixed: Bool = true) -> String {
        let (r, g, b, _) = self.solo_rgbaComponents
    ...
    /// - Returns: `String`
    func solo_shortHexOrLong() -> String {
        let long = self.solo_hexString(prefixed: false)
...
public extension UIColor {
    ...
    static func solo_complementary(for color: UIColor) -> UIColor? {
        var r: CGFloat = 0, g: CGFloat = 0, b: CGFloat = 0, a: CGFloat = 0
    ...
    var solo_complementary: UIColor {
    ...
    /// 增加亮度
    func solo_lighten(by amount: CGFloat = 0.2) -> UIColor {
        var h: CGFloat = 0, s: CGFloat = 0, b: CGFloat = 0, a: CGFloat = 0
        self.getHue(&h, saturation: &s, brightness: &b, alpha: &a)
    ...
    /// 降低亮度
    func solo_darken(by amount: CGFloat = 0.2) -> UIColor {
        var h: CGFloat = 0, s: CGFloat = 0, b: CGFloat = 0, a: CGFloat = 0
        self.getHue(&h, saturation: &s, brightness: &b, alpha: &a)
    ...
    /// 确保饱和度不低于指定值
    func solo_withMinSaturation(_ minSaturation: CGFloat) -> UIColor {
        var h: CGFloat = 0, s: CGFloat = 0, b: CGFloat = 0, a: CGFloat = 0
        self.getHue(&h, saturation: &s, brightness: &b, alpha: &a)
    ...
    /// 混合两个颜色(按权重)
    static func solo_blend(_ color1: UIColor, weight1: CGFloat = 0.5, with color2: UIColor, weight2: CGFloat = 0.5) -> UIColor {
        let total = weight1 + weight2
        let w1 = weight1 / total
        let w2 = weight2 / total

        let (r1, g1, b1, a1) = color1.solo_rgbaComponents
        let (r2, g2, b2, a2) = color2.solo_rgbaComponents
    ...
    /// 在两个颜色之间插值
    static func solo_interpolate(from start: UIColor, to end: UIColor, progress: CGFloat) -> UIColor {
        let p = min(max(progress, 0), 1)
        let (sr, sg, sb, sa) = start.solo_rgbaComponents
        let (er, eg, eb, ea) = end.solo_rgbaComponents
        return UIColor(
            red: sr + (er - sr) * p,
            green: sg + (eg - sg) * p,
            blue: sb + (eb - sb) * p,
...
public extension UIColor {
    ...
    var solo_isDark: Bool {
        let components = self.solo_rgbComponents
    ...
    func solo_isDistinct(from color: UIColor, threshold: CGFloat = 0.25) -> Bool {
        let bg = self.solo_rgbComponents
        let fg = color.solo_rgbComponents

        // 确保两个颜色都不是灰色系
        if (bg.red - bg.green).solo_abs() < 0.03 && (bg.red - bg.blue).solo_abs() < 0.03 &&
            (fg.red - fg.green).solo_abs() < 0.03 && (fg.red - fg.blue).solo_abs() < 0.03
        {
            return false
        }

        return (bg.red - fg.red).solo_abs() > threshold ||
            (bg.green - fg.green).solo_abs() > threshold ||
            (bg.blue - fg.blue).solo_abs() > threshold
    }
    ...
    func solo_hasSufficientContrast(with color: UIColor) -> Bool {
        let bg = self.solo_rgbComponents
        let fg = color.solo_rgbComponents
...
public extension UIColor {
    ...
    static func solo_dynamic(hex: String) -> UIColor {
        let color = self.solo_color(from: hex)
        return self.solo_dynamic(light: color, dark: color)
    }
    ...
    ///   - darkHex: 深色模式下的颜色
    /// - Returns: 动态颜色对象
    static func solo_dynamic(lightHex: String, darkHex: String) -> UIColor {
        let light = self.solo_color(from: lightHex)
        let dark = self.solo_color(from: darkHex)
        return self.solo_dynamic(light: light, dark: dark)
    }

    /// 使用相同的 `UIColor` 创建动态颜色
    static func solo_dynamic(color: UIColor) -> UIColor {
        self.solo_dynamic(light: color, dark: color)
    }
    ...
    ///   - dark: 深色模式颜色
    /// - Returns: 动态颜色
    static func solo_dynamic(light: UIColor, dark: UIColor) -> UIColor {
...
public extension UIColor {
    ...
    static func solo_gradientLayer(
        frame: CGRect,
        colors: [UIColor],
    ...
    static func solo_gradientColor(
        size: CGSize,
        colors: [UIColor],
        locations: [CGFloat]? = nil,
        startPoint: CGPoint = .zero,
        ...
        guard let image = self.solo_gradientImage(
            size: size,
            colors: colors,
            locations: locations,
...
public extension [UIColor] {
    /// 生成渐变图层
    func solo_gradientLayer(
        frame: CGRect,
        locations: [CGFloat]? = nil,
        startPoint: CGPoint = .zero,
        endPoint: CGPoint = CGPoint(x: 1, y: 1)
    ) -> CAGradientLayer {
        UIColor.solo_gradientLayer(
            frame: frame,
            colors: self,
            locations: locations,
            startPoint: startPoint,
    ...
    /// 生成渐变图片
    func solo_gradientImage(
        size: CGSize,
        locations: [CGFloat]? = nil,
        startPoint: CGPoint = .zero,
        endPoint: CGPoint = CGPoint(x: 1, y: 1)
    ) -> UIImage? {
        UIColor.solo_gradientImage(
            size: size,
            colors: self,
            locations: locations,
            startPoint: startPoint,
    ...
    /// 生成渐变 `UIColor`(谨慎使用)
    func solo_gradientColor(
        size: CGSize,
        locations: [CGFloat]? = nil,
        startPoint: CGPoint = .zero,
        endPoint: CGPoint = CGPoint(x: 1, y: 1)
    ) -> UIColor? {
        UIColor.solo_gradientColor(
            size: size,
            colors: self,
            locations: locations,
            startPoint: startPoint,
