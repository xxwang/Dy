...
public extension UIView {
    ...
    private func dy_constraint(for attribute: NSLayoutConstraint.Attribute) -> NSLayoutConstraint? {
        ...
        if let constraint = self.constraints.first(where: { constraint in
            self.dy_isConstraint(constraint, affecting: self, with: attribute)
        }) {
            return constraint
        }
