...
extension UIView {
    ...
    /// 角标Label
    var dy_badgeLabel: UILabel? {
        get { self.dy_GetAO(forKey: &DyKeys.badgeLabel) }
        set { self.dy_SetAO(newValue, forKey: &DyKeys.badgeLabel, policy: .OBJC_ASSOCIATION_RETAIN_NONATOMIC) }
    }

    /// 水印配置属性
    var dy_watermarkConfig: DyWatermarkConfig? {
        get { self.dy_GetAO(forKey: &DyKeys.watermark) }
        set { self.dy_SetAO(newValue, forKey: &DyKeys.watermark, policy: .OBJC_ASSOCIATION_RETAIN_NONATOMIC) }
    }
}
...
public extension UIView {
    ...
    var dy_width: CGFloat {
        ...
        set { self.frame = CGRect(origin: self.dy_origin, size: CGSize(width: newValue, height: self.dy_height)) }
    }

    /// 视图的高度
    var dy_height: CGFloat {
        get { return self.frame.height }
        set { self.frame = CGRect(origin: self.dy_origin, size: CGSize(width: self.dy_width, height: newValue)) }
    }

    /// 视图的顶部位置 (等同于 `y`)
    var dy_top: CGFloat {
        get { return self.frame.origin.y }
        set { self.frame = CGRect(origin: CGPoint(x: self.dy_left, y: newValue), size: self.dy_size) }
    }

    /// 视图的左侧位置 (等同于 `x`)
    var dy_left: CGFloat {
        get { return self.frame.origin.x }
        set { self.frame = CGRect(origin: CGPoint(x: newValue, y: self.dy_top), size: self.dy_size) }
    }

    /// 视图中心点的 x 坐标
    var dy_centerX: CGFloat {
        get { return self.center.x }
        set { self.center = CGPoint(x: newValue, y: self.dy_centerY) }
    }
...
public extension UIView {
    ...
    func dy_debugHighlight(
        ...
        backgroundColor: UIColor = .dy_random.dy_alpha(0.2)
    ) {
        #if DEBUG
            // 高亮当前视图
