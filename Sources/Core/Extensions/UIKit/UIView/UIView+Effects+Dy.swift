...
extension UIView {
    ...
    var dy_emitterOriginalRates: [ObjectIdentifier: Float] {
        get { self.dy_GetAO(forKey: &DyKeys.emitterOriginalRatesKey) as? [ObjectIdentifier: Float] ?? [:] }
        set { self.dy_SetAO(newValue, forKey: &DyKeys.emitterOriginalRatesKey, policy: .OBJC_ASSOCIATION_RETAIN) }
    }
}
...
public extension UIView {
    ...
    func dy_showBadge(_ number: String, position: CGPoint = CGPoint(x: 1, y: 0)) {
        ...
        if self.dy_badgeLabel == nil {
            ...
            self.dy_badgeLabel = label
        }

        guard let label = self.dy_badgeLabel else {
    ...
    /// 移除角标
    func dy_removeBadge() {
        self.dy_badgeLabel?.removeFromSuperview()
        self.dy_badgeLabel = nil
    }
}
