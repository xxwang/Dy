...
public extension UIColor {
    ...
    /// 转换为`CIColor`
    func dy_cIColor() -> CoreImage.CIColor {
    ...
    /// - Parameter size: 图片尺寸
    /// - Returns: `UIImage?`
    func dy_uIImage(ofSize size: CGSize) -> UIImage? {
    ...
    func dy_hexString(prefixed: Bool = true) -> String {
        let (r, g, b, _) = self.dy_rgbaComponents
    ...
    /// - Returns: `String`
    func dy_shortHexOrLong() -> String {
        let long = self.dy_hexString(prefixed: false)
...
public extension UIColor {
    ...
    func dy_isDistinct(from color: UIColor, threshold: CGFloat = 0.25) -> Bool {
        ...
        // 确保两个颜色都不是灰色系
        if (bg.red - bg.green).dy_abs() < 0.03 && (bg.red - bg.blue).dy_abs() < 0.03 &&
            (fg.red - fg.green).dy_abs() < 0.03 && (fg.red - fg.blue).dy_abs() < 0.03
        {
            return false
        }

        return (bg.red - fg.red).dy_abs() > threshold ||
            (bg.green - fg.green).dy_abs() > threshold ||
            (bg.blue - fg.blue).dy_abs() > threshold
    }
...
public extension UIColor {
    ...
    static func dy_dynamic(hex: String) -> UIColor {
        let color = self.dy_color(from: hex)
    ...
    static func dy_dynamic(lightHex: String, darkHex: String) -> UIColor {
        let light = self.dy_color(from: lightHex)
        let dark = self.dy_color(from: darkHex)
...
public extension UIColor {
    ...
    static func dy_gradientColor(
        ...
        guard let image = self.dy_gradientImage(
            size: size,
            colors: colors,
            locations: locations,
...
public extension [UIColor] {
    /// 生成渐变图层
    func dy_gradientLayer(
        frame: CGRect,
        locations: [CGFloat]? = nil,
        startPoint: CGPoint = .zero,
        endPoint: CGPoint = CGPoint(x: 1, y: 1)
    ) -> CAGradientLayer {
        UIColor.dy_gradientLayer(
            frame: frame,
            colors: self,
            locations: locations,
            startPoint: startPoint,
    ...
    /// 生成渐变图片
    func dy_gradientImage(
        size: CGSize,
        locations: [CGFloat]? = nil,
        startPoint: CGPoint = .zero,
        endPoint: CGPoint = CGPoint(x: 1, y: 1)
    ) -> UIImage? {
        UIColor.dy_gradientImage(
            size: size,
            colors: self,
            locations: locations,
            startPoint: startPoint,
    ...
    /// 生成渐变 `UIColor`(谨慎使用)
    func dy_gradientColor(
        size: CGSize,
        locations: [CGFloat]? = nil,
        startPoint: CGPoint = .zero,
        endPoint: CGPoint = CGPoint(x: 1, y: 1)
    ) -> UIColor? {
        UIColor.dy_gradientColor(
            size: size,
            colors: self,
            locations: locations,
            startPoint: startPoint,
