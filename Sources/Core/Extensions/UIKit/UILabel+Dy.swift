...
public extension UILabel {
    ...
    var dy_requiredHeight: CGFloat {
        ...
        if self.numberOfLines > 0 {
            ...
            if boundingRect.height > maxHeight {
                boundingRect.size.height = maxHeight.dy_ceil()
            }
        }

        return boundingRect.height.dy_ceil()
    }
    ...
    var dy_allTextLines: [String] {
        ...
        return text.dy_wrappedLines(maxWidth: self.bounds.width, font: font)
    }
...
public extension UILabel {
    ...
    ///   let size = label.dy_viewSize(maxWidth: 200)
    ///   ```
    func dy_viewSize(maxWidth: CGFloat = .greatestFiniteMagnitude) -> CGSize {
        return if self.attributedText != nil {
            self.attributedText?.dy_viewSize(maxWidth: maxWidth) ?? .zero
        } else {
            self.text?.dy_viewSize(maxWidth: maxWidth, font: self.font) ?? .zero
        }
    }
    ...
    /// - Returns: `CGSize`
    func dy_viewSizeThatFits(maxWidth: CGFloat) -> CGSize {
