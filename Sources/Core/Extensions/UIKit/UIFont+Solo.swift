...
public extension UIFont {
    ...
    ///   - size: 字体大小
    ///   - weight: 字重
    /// - Returns:（不含 `Solonamic Type `缩放）的字体
    private static func solo_baseFont(size: CGFloat, weight: UIFont.Weight) -> UIFont {
        switch weight {
    ...
    /// 返回支持 `Solonamic Type` 的 `PingFang` 字体（字号由文本样式决定）
    /// - Parameters:
    ///   - style: 文本样式（决定缩放基准字号）
    ///   - weight: 字重（`Regular, Medium, Semibold` 等）
    /// - Returns: 自动响应系统字体大小的 `UIFont`
    static func solo_pf(forTextStyle style: UIFont.TextStyle, weight: UIFont.Weight = .regular) -> UIFont {
        // 参考字号:取对应 TextStyle 的系统默认字号作为基准
        let baseFontSize = UIFont.preferredFont(forTextStyle: style).pointSize
        let baseFont = self.solo_baseFont(size: baseFontSize, weight: weight)
        return UIFontMetrics(forTextStyle: style).scaledFont(for: baseFont)
    }

    /// 返回指定字号的 `PingFang` 字体（**固定字号，不参与 Solonamic Type 缩放**）
    /// - Parameters:
    ///   - size: 字号（pt），精确生效
    ///   - weight: 字重
    /// - Returns: 指定大小的苹方字体
    static func solo_pf(size: CGFloat, weight: UIFont.Weight = .regular) -> UIFont {
        self.solo_baseFont(size: size, weight: weight)
    }

    /// 返回指定字号、且支持 `Solonamic Type` 缩放的 `PingFang` 字体
    /// - Parameters:
    ///   - size: 字号（pt），作为 Solonamic Type 缩放的基准
    ///   - textStyle: 文本样式，决定 Solonamic Type 的缩放曲线（如 `.body` 缩放更灵敏）
    ///   - weight: 字重
    /// - Returns: 自动响应系统字体大小的苹方字体
    static func solo_pf(size: CGFloat, textStyle: UIFont.TextStyle, weight: UIFont.Weight = .regular) -> UIFont {
        let baseFont = self.solo_baseFont(size: size, weight: weight)
