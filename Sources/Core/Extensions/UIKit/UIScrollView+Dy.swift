...
public extension DyWrapper where Base: UIScrollView {
    ...
    func captureVisibleScreenshot(options: DyScreenshotOptions = DyScreenshotOptions()) -> UIImage? {
    ...
    func captureFullScreenshot(options: DyScreenshotOptions = DyScreenshotOptions(),
                               completion: @escaping DyAction1<UIImage?>)
    {
        ...
        let totalPages = Int((totalSize.height / pageSize.height).dy.ceil())
        ...
        func renderPage(index: Int) {
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
                ...
                if index < totalPages - 1 {
                    ...
                    completion(image?.dy.compress(qualityRange: options.qualityRange))
                }
            }
        }
        renderPage(index: 0)
