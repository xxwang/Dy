...
extension UIGestureRecognizer {
    ...
    var dy_recognizedBlock: DyAction1<UIGestureRecognizer>? {
        get { return self.dy_GetAO(forKey: &DyKeys.recognized) }
        set { self.dy_SetAO(newValue, forKey: &DyKeys.recognized) }
    }

    /// 手势状态变化时触发的闭包
    var dy_stateChangedBlock: DyAction1<UIGestureRecognizer.State>? {
        get { return self.dy_GetAO(forKey: &DyKeys.stateChanged) }
        set { self.dy_SetAO(newValue, forKey: &DyKeys.stateChanged) }
    }
