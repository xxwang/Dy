...
public extension SoloWrapper where Base: UICollectionView {
    ...
    func allowMoveItem() {
        ...
        base.solo.SetAO(longPress, forKey: &Base.Keys.moveItemGestureKey)
    }

    /// 禁用拖拽移动功能(仅移除 `allowMoveItem` 添加的长按手势,不影响其它长按手势)
    func disableMoveItem() {
        if let gesture = base.solo.GetAO(forKey: &Base.Keys.moveItemGestureKey) as? UILongPressGestureRecognizer {
            base.removeGestureRecognizer(gesture)
            base.solo.SetAO(nil, forKey: &Base.Keys.moveItemGestureKey)
        }
    }
}
