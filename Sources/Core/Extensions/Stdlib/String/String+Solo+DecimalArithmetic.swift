...
// MARK: 私有辅助工具
public extension SoloWrapper where Base == String {
    private func performOperation(_ other: String?, _ operation: SoloFunc2<NSDecimalNumber, NSDecimalNumber, NSDecimalNumber>) -> String {
        let left = self.toNSDecimalNumber()
        let right = (other ?? "").solo.toNSDecimalNumber()
