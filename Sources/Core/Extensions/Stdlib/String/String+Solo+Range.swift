...
public extension String {
    ...
    var solo_fullRange: Range<String.Index> {
    ...
    var solo_fullNSRange: NSRange {
        NSRange(self.solo_fullRange, in: self)
    }
    ...
    func solo_range(from nsRange: NSRange) -> Range<String.Index>? {
    ...
    func solo_nsRange(from range: Range<String.Index>) -> NSRange {
    ...
    func solo_subRange(of substring: String) -> Range<String.Index>? {
    ...
    func solo_subNSRange(of substring: String) -> NSRange {
    ...
    func solo_ranges(of substring: String, options: String.CompareOptions = []) -> [Range<String.Index>] {
    ...
    func solo_nsRanges(of substring: String, options: String.CompareOptions = []) -> [NSRange] {
        self.solo_ranges(of: substring, options: options).map { self.solo_nsRange(from: $0) }
    }
}
...
public extension String {
    ...
    func solo_padStart(toLength length: Int, with padding: String = " ") -> String {
    ...
    func solo_padEnd(toLength length: Int, with padding: String = " ") -> String {
        guard length > self.count, !padding.isEmpty else { return self }
        let needed = length - self.count
        let repeatCount = (needed + padding.count - 1) / padding.count
        let repeated = String(repeating: padding, count: repeatCount)
...
public extension String {
    ...
    func solo_positionFirst(of substring: String) -> Int {
        return self.solo_position(of: substring, backwards: false)
    }
    ...
    func solo_positionLast(of substring: String) -> Int {
        return self.solo_position(of: substring, backwards: true)
    }

    /// 私有辅助方法：统一实现前后向查找
    private func solo_position(of substring: String, backwards: Bool) -> Int {
...
public extension String {
    ...
    func solo_slice(_ range: Range<Int>) -> String {
        guard !range.isEmpty,
              range.lowerBound >= 0,
    ...
    func solo_substring(from: Int) -> String {
        self.solo_slice(from ..< self.utf16.count)
    }
    ...
    func solo_substring(to: Int) -> String {
        let end = min(max(0, to), self.utf16.count)
        return self.solo_slice(0 ..< end)
    }
    ...
    func solo_substring(from: Int, length: Int) -> String {
        guard length > 0 else { return "" }
        let start = max(0, from)
        let end = min(start + length, self.utf16.count)
        return self.solo_slice(start ..< end)
    }
    ...
    ///   - to: 结束索引（不包含）
    /// - Returns: 有效范围内的子串
    func solo_substring(from: Int, to: Int) -> String {
    ...
    func solo_character(at index: Int) -> String {
        self.solo_substring(from: index, length: 1)
    }
    ...
    func solo_truncate(length: Int) -> String {
        guard length < self.utf16.count else { return self }
        return self.solo_substring(to: length)
    }
    ...
    func solo_truncate(length: Int, trailing: String = "...") -> String {
        guard length >= 0, self.utf16.count > length else { return self }
        let truncated = self.solo_substring(to: length)
    ...
    ///   - separator: 分隔符,默认为 `"-"`
    /// - Returns: 分段拼接后的字符串
    func solo_chunked(segmentLength: Int, separator: String = "-") -> String {
