...
public extension SoloWrapper where Base == String {
    ...
    var isSingleEmoji: Bool {
        return base.count == 1 && base.first?.solo.isEmoji == true
    }

    /// 判断字符串是否包含至少一个 Emoji 字符
    var containsEmoji: Bool {
        return base.contains { $0.solo.isEmoji }
    }

    /// 判断字符串是否仅由`Emoji`字符组成（不含空格、标点等）
    var containsOnlyEmoji: Bool {
        return !base.isEmpty && base.allSatisfy(\.solo.isEmoji)
    }
    ...
    var emojis: [Character] {
        return base.filter(\.solo.isEmoji)
    }
