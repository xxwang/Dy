...
public extension DyWrapper where Base == String {
    ...
    var isValidHttpsURL: Bool {
        URL(string: base)?.scheme == "https"
    }

    /// 是否为 HTTP URL
    var isValidHttpURL: Bool {
        URL(string: base)?.scheme == "http"
    }

    /// 是否为文件 URL
    var isValidFileURL: Bool {
        URL(string: base)?.isFileURL == true
    }
}
...
public extension DyWrapper where Base == String {
    ...
    func isValidAlphanumeric(minLen: Int, maxLen: Int) -> Bool {
        guard base.count >= minLen, base.count <= maxLen else { return false }
        return base.allSatisfy { $0.isLetter || $0.isNumber || $0 == "_" }
    }
    ...
    var isValidUsername: Bool {
        base.count >= 1 && base.count <= 20 && base.allSatisfy { $0.isLetter || ($0.unicodeScalars.first?.value ?? 0) >= 0x4E00 }
    }

    /// 是否为有效密码
    /// - `complex = false`: 至少包含字母+数字,≥6 位
    /// - `complex = true`: 必须包含大小写字母+数字+特殊符号,≥8 位
    func isValidPassword(complex: Bool = false) -> Bool {
        if complex {
            return isMatch(pattern: #"^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[!@#$%^&*()_+\-=\[\]{}|;:'",.<>/?]).{8,}$"#)
        } else {
            return base.count >= 6 && self.hasLetters && self.hasDigits
        }
    }
}
...
public extension DyWrapper where Base == String {
    /// 是否符合身份证基本格式(15/18 位)
    var isBasicIDNumber: Bool {
        isMatch(pattern: #"^(\d{15}|\d{17}[\dXx])$"#)
    }

    /// 是否为严格有效的 18 位身份证(含校验码)
    var isStrictIDNumber: Bool {
        guard base.count == 18, isBasicIDNumber else { return false }
        ...
        let actual = String(base[base.index(base.startIndex, offsetBy: 17)])
...
public extension DyWrapper where Base == String {
    ...
    func contains(_ substring: String, caseSensitive: Bool = true) -> Bool {
        ...
        return base.localizedCaseInsensitiveContains(substring)
    }
