...
public extension String {
    ...
    ///
    func solo_htmlToAttributedString(
        ...
        guard let data = fullHTML.data(using: .utf8) else {
            return self.solo_fallbackAttributedString(font: font, lineSpacing: lineSpacing)
        }
    ...
    // MARK: - 私有辅助方法
    private func solo_fallbackAttributedString(font: UIFont?, lineSpacing: CGFloat?) -> NSMutableAttributedString {
        let attr = NSMutableAttributedString(string: self)
        if let font {
