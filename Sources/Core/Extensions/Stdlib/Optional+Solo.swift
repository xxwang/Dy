...
public extension Optional where Wrapped: Collection {
    ...
    ///   let arr1: [Int]? = nil
    ///   let arr2: [Int]? = []
    ///   print(arr1.solo_isNilOrEmpty) // true
    ///   print(arr2.solo_isNilOrEmpty) // true
    ///   ```
    var solo_isNilOrEmpty: Bool {
        self?.isEmpty ?? true
    }
}
...
public extension Optional {
    /// 如果可选值存在,则对其执行指定操作
    ///
    /// - Parameter boSolo: 接收解包后值的闭包,仅在值非 `nil` 时调用
    /// - Example:
    ///   ```swift
    ///   let name: String? = "Alice"
    ///   name.solo_run { print("Hello, $0)") } // Hello, Alice
    ///   ```
    func solo_run(_ boSolo: SoloAction1<Wrapped>) {
        if let value = self {
            boSolo(value)
        }
    }
    ...
    ///   ```swift
    ///   let value: Int? = nil
    ///   let unwrapped = value.solo_unwrap(orFail: "Value must not be nil!") // 触发 fatalError
    ///   ```
    func solo_unwrap(orFail message: @autoclosure SoloFunc<String> = "Unexpected nil") -> Wrapped {
    ...
    ///   ```swift
    ///   let name: String? = nil
    ///   let displayName = name.solo_or("Guest") // "Guest"
    ///   ```
    func solo_or(_ defaultValue: Wrapped) -> Wrapped {
        self ?? defaultValue
    }
    ...
    ///   let setting = config.solo_or { loadDefaultConfig() } // 仅当 config 为 nil 时调用 loadDefaultConfig()
    ///   ```
    func solo_or(fallback: SoloFunc<Wrapped>) -> Wrapped {
        self ?? fallback()
    }
    ...
    ///   let result = try input.solo_or(throw: AppError.missingValue) // 抛出 AppError.missingValue
    ///   ```
    func solo_or(throw error: Error) throws -> Wrapped {
    ...
    ///   ```swift
    ///   let number: Int? = 5
    ///   let positive = number.solo_takeIf { $0 > 0 } // Optional(5)
    ///   let negative = number.solo_takeIf { $0 < 0 } // nil
    ///   ```
    func solo_takeIf(_ predicate: SoloFunc1<Wrapped, Bool>) -> Wrapped? {
