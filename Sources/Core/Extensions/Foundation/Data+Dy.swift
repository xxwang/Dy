...
public extension Data {
    ...
    ///   print(data.dy_hexString())  // "A1B2"
    ///   ```
    func dy_hexString() -> String {
...
public extension Data {
    ...
    ///   let encoded = original.dy_encodebase64()  // Data of "SGVsbG8="
    ///   ```
    func dy_encodebase64() -> Data? {
    ...
    ///   let decoded = self64Bytes.dy_decodebase64()  // Data of "Hello"
    ///   ```
    /// - Recommendation: 更常见的做法是 `Data(base64Encoded: base64String)`
    func dy_decodebase64() -> Data? {
...
public extension Data {
    ...
    ///   let str = data.dy_string()  // "Swift"
    ///   ```
    func dy_string(encoding: String.Encoding = .utf8) -> String? {
    ...
    ///   """.data(using: .utf8)!
    ///
    ///   if let dict: [String: Any] = json.dy_object() {
    ///       print(dict["name"] as? String ?? "")  // "Alice"
    ///   }
    ///   ```
    func dy_object<T>(for type: T.Type = [String: Any].self, options: JSONSerialization.ReadingOptions = []) -> T? {
