...
public extension Calendar {
    ...
    func dy_endOfMonth(year: Int, month: Int) -> Date? {
        guard let start = self.dy_startOfMonth(year: year, month: month),
...
public extension Calendar {
    ...
    func dy_datesInWeek(for date: Date = Date()) -> [Date] {
        guard let start = self.dy_startOfWeek(for: date) else { return [] }
...
public extension Calendar {
    ...
    func dy_endOfMonth(for date: Date = Date()) -> Date? {
        guard let startOfMonth = self.dy_startOfMonth(for: date),
