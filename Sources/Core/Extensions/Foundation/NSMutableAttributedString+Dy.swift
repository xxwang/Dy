...
public extension NSMutableAttributedString {
    ...
    ///   ```
    @discardableResult
    func dy_attachment(_ image: UIImage?, bounds: CGRect = .zero, at index: Int = 0) -> Self {
        ...
        if bounds != .zero {
            ...
            if index < self.length {
                if let existingFont = self.attribute(.font, at: index, effectiveRange: nil) as? UIFont {
                    fontSize = existingFont.pointSize
                }
            } else if self.length > 0 {
                // 如果插入到末尾,取最后一个字符的字体
                if let existingFont = self.attribute(.font, at: self.length - 1, effectiveRange: nil) as? UIFont {
                    fontSize = existingFont.pointSize
                }
            }
        ...
        self.insert(imageAttrStr, at: index)
    ...
    /// - Returns: `Self`
    @discardableResult
    func dy_font(_ font: UIFont?, for range: NSRange? = nil) -> Self {
        guard let font else { return self }
        let range = range ?? self.dy_fullNSRange
    ...
    @discardableResult
    func dy_characterSpacing(_ spacing: CGFloat, for range: NSRange? = nil) -> Self {
        let range = range ?? self.dy_fullNSRange
    ...
    /// - Returns: `Self`
    @discardableResult
    func dy_lineSpacing(_ lineSpacing: CGFloat, alignment: NSTextAlignment = .left, for range: NSRange? = nil) -> Self {
        let range = range ?? self.dy_fullNSRange
    ...
    @discardableResult
    func dy_fixedLineHeight(_ lineHeight: CGFloat, alignment: NSTextAlignment = .left, for range: NSRange? = nil) -> Self {
        let range = range ?? self.dy_fullNSRange
        let style = NSMutableParagraphStyle()
    ...
    /// - Returns: `Self`
    @discardableResult
    func dy_paragraphSpacing(_ spacing: CGFloat, for range: NSRange? = nil) -> Self {
        let range = range ?? self.dy_fullNSRange
        let style = NSMutableParagraphStyle()
    ...
    /// - Returns: `Self`
    @discardableResult
    func dy_firstLineHeadIndent(_ indent: CGFloat) -> Self {
        let style = NSMutableParagraphStyle()
        style.firstLineHeadIndent = indent
        self.addAttribute(.paragraphStyle, value: style, range: self.dy_fullNSRange)
        return self
    }
    ...
    /// - Returns: `Self`
    @discardableResult
    func dy_foregroundColor(_ color: UIColor, for range: NSRange? = nil) -> Self {
        let range = range ?? self.dy_fullNSRange
    ...
    /// - Returns: `Self`
    @discardableResult
    func dy_backgroundColor(_ color: UIColor, for range: NSRange? = nil) -> Self {
        let range = range ?? self.dy_fullNSRange
    ...
    /// - Returns: `Self`
    @discardableResult
    func dy_underline(color: UIColor, style: NSUnderlineStyle = .single, for range: NSRange? = nil) -> Self {
        let range = range ?? self.dy_fullNSRange
    ...
    /// - Returns: `Self`
    @discardableResult
    func dy_strikethrough(color: UIColor, style: NSUnderlineStyle = .single, for range: NSRange? = nil) -> Self {
        let range = range ?? self.dy_fullNSRange
    ...
    /// - Returns: `Self`
    @discardableResult
    func dy_obliqueness(_ factor: Float = 0, for range: NSRange? = nil) -> Self {
        let range = range ?? self.dy_fullNSRange
    ...
    /// - Returns: `Self`
    @discardableResult
    func dy_expansion(_ factor: Float = 1.0, for range: NSRange? = nil) -> Self {
        let range = range ?? self.dy_fullNSRange
    ...
    ///   ```
    @discardableResult
    func dy_textShadow(color: UIColor, offset: CGSize, radius: CGFloat, for range: NSRange? = nil) -> Self {
        let range = range ?? self.dy_fullNSRange
        let shadow = NSShadow()
...
public extension NSMutableAttributedString {
    ...
    ///   ```
    @discardableResult
    func dy_addAttributes(_ attributes: [NSAttributedString.Key: Any], toOccurrencesOf target: some StringProtocol) -> Self {
        // 使用 \Q...\E 转义目标字符串中的正则元字符,实现字面匹配
        let pattern = "\\Q\(target)\\E"
        return self.dy_addAttributes(attributes, toRangesMatching: pattern)
    }
    ...
    ///   ```
    @discardableResult
    func dy_addAttributes(_ attributes: [NSAttributedString.Key: Any], toRangesMatching pattern: String, options: NSRegularExpression.Options = []) -> Self {
        guard !pattern.isEmpty,
              let regex = try? NSRegularExpression(pattern: pattern, options: options),
              self.length > 0 else { return self }

        let matches = regex.matches(in: self.string, options: [], range: self.dy_fullNSRange)
        for match in matches {
            self.dy_addAttributes(attributes, for: match.range)
        }
        return self
    }
    ...
    /// - Returns: `Self`
    @discardableResult
    func dy_addAttributes(_ attributes: [NSAttributedString.Key: Any], for range: NSRange) -> Self {
        let isValidRange = range.location >= 0 && range.length >= 0 && range.location + range.length <= self.length
        if isValidRange {
    ...
    /// 安全地添加单个属性,自动忽略无效范围
    func dy_addAttribute(_ name: NSAttributedString.Key, value: Any, range: NSRange) {
        let isValidRange = range.location >= 0 && range.length >= 0 && range.location + range.length <= self.length
        if isValidRange {
...
public extension NSMutableAttributedString {
    ...
    func dy_string(_ string: String) -> Self {
        self.setAttributedString(string.dy_toNSAttributedString())
        return self
    }
    ...
    /// - Returns: `Self`
    @discardableResult
    func dy_attributedString(_ attributedString: NSAttributedString) -> Self {
    ...
    /// - Returns: `Self`
    @discardableResult
    func dy_append(_ attributedString: NSAttributedString) -> Self {
