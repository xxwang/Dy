...
public extension DyLoadable where Self: UIView {
    ...
    /// - Returns: 加载的视图实例
    static func dy_loadView(
        nibName: String? = nil,
        bundle: Bundle? = nil
    ) -> Self {
        let name = nibName ?? DyHelper.shared.className(Self.self)
...
public extension DyLoadable where Self: UIViewController {
    ...
    /// - Returns: 加载的控制器实例
    static func dy_loadViewController(
        from storyboardName: String = "Main",
        bundle: Bundle? = nil,
        identifier: String? = nil
    ) -> Self {
        let storyboardId = identifier ?? DyHelper.shared.className(self)
