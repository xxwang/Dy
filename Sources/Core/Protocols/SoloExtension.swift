...
// MARK: - 命名空间
public protocol SoloExtension {
    associatedtype WrapperType
    var solo: WrapperType { get }
}

// MARK: - 提供命名空间入口
public extension SoloExtension {
    /// 实例命名空间入口
    var solo: SoloWrapper<Self> {
        SoloWrapper(self)
    }

    /// 类型命名空间入口
    static var solo: SoloWrapper<Self>.Type {
        SoloWrapper<Self>.self
    }
}

// MARK: - 配置包装器
public final class SoloWrapper<Base> {
    /// 被包装的原始实例
...
// MARK: - 作用于引用类型
public extension SoloWrapper where Base: AnyObject {
