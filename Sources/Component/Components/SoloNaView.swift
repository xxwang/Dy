...
open class SoloNaView: UIView {
    /// 返回按钮点击回调
    open var backBlock: SoloAction?
    ...
    override open func layoutSubviews() {
        ...
        statusBar.frame = CGRect(
            ...
            y: 0,
            width: bounds.width,
            height: SoloScreen.statusBarHeight
        )
        ...
        titleBar.frame = CGRect(
            ...
            width: bounds.width,
            height: SoloScreen.navigationBarHeight
        )

        // 分割线：位于整个导航栏底部
        let lineHeight = 1.0 / SoloScreen.screenScale
        ...
        let buttonX = max(10, SoloScreen.safeAreaLeft)
        ...
        let rightInset = max(10, SoloScreen.safeAreaRight)
...
// MARK: - 工厂方法
public extension SoloNaView {
    /// 创建一个带有默认样式的导航栏实例
    ///
    /// - Returns: 配置好的 `SoloNaView`
    ///
    /// - Example:
    ///   ```swift
    ///   let navBar = SoloNaView.naview()
    ///       .solo
    ///       .title("首页")
    ///       .backAction { /* 返回逻辑 */ }
    ///       .build()
    ///   ```
    static func naview() -> SoloNaView {
        let naView = SoloNaView()
            .solo
            .size(CGSize(
                width: SoloScreen.screenWidth,
                height: SoloScreen.navBarTotalHeight
            ))
            .showLine(true)
            .showShadow(false)
            .titleColor(.black)
...
// MARK: - 链式设置
public extension SoloWrapper where Base: SoloNaView {
    /// 设置标题文本
    @discardableResult
    ...
    @discardableResult
    func backAction(_ handler: SoloAction?) -> Self {
